...
export default function SignInScreen() {
  ...
  const isVerifying = awaitingCode || signIn.status === 'needs_first_factor';

  return (
    <ClerkAuthShell
      mode="sign-in"
      stage={isVerifying ? 'verify' : 'request'}
      title={isVerifying ? 'Check your email' : 'Sign in'}
      statusLabel={isVerifying ? `Code sent to ${emailAddress || 'your inbox'}` : undefined}
      subtitle={
        isVerifying
          ? 'Enter the one-time code we sent to finish signing in on this device.'
